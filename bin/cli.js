...
function getFlagValue(label, flags, index) {
...
function parseCreateArgs() {
  ...
  if (
    process.argv.slice(3).includes("-h") ||
  ...
  while (i < process.argv.length) {
    switch (process.argv[i]) {
      case "-d":
      case "--directory":
        opts.directory = getFlagValue("Directory", "--directory, -d", i);
        i += 2;
        break;
      case "-n":
      case "--name":
        opts.name = getFlagValue("Name", "--name, -n", i).replace(/\.sql$/, "");
        i += 2;
        break;
      case "-p":
      case "--pad-width": {
        const padWidthValue = getFlagValue("Pad width", "--pad-width, -p", i);
